...
from Utils.helpers import pretty_dt
...
def ErrorHandle(bot: commands.Bot, logging_channel: int = None):
    @bot.event
    async def on_command_error(ctx, error):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            ...
            if isinstance(error.__cause__, discord.Forbidden):
                ...
                if logging_channel and (channel := bot.get_channel(logging_channel)):
                    errorMsg = f"**Uncaught Error**:\nCommand: `{ctx.command.name}`\nError: `{type(error).__name__}`:```\n{error}```"
